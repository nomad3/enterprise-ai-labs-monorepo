...
from agentprovision.core.services.agent_orchestrator import (
    AgentOrchestrator, AgentTask, TaskPriority, TaskStatus, WorkflowDefinition,
    get_orchestrator)
...
class AgentScalingRequest(BaseModel):
    ...
    max_concurrent_tasks: int = Field(
        ..., ge=1, le=100, description="Maximum concurrent tasks"
    )
...
async def pause_agent(
    ...
    try:
        success = await orchestrator.pause_agent(agent_id, db)
...
async def get_orchestration_metrics(
    ...
    try:
        ...
        total_queued_tasks = sum(
            len(queue) for queue in orchestrator.task_queue.values()
        )
        total_running_tasks = len(orchestrator.running_tasks)

        # Calculate utilization
        utilization = (
            (total_current_tasks / total_max_tasks * 100) if total_max_tasks > 0 else 0
        )
