...
def test_auth_flow():
    ...
    verify_response = requests.get(
        f"{BASE_URL}/auth/verify", headers={"Authorization": f"Bearer {token}"}
    )
...
def test_ticket_flow(token: str):
    ...
    create_response = requests.post(
        f"{BASE_URL}/tickets", json=ticket_data, headers=headers
    )
    assert create_response.status_code == 200
    ticket_id = create_response.json()["id"]

    # Get ticket
    get_response = requests.get(f"{BASE_URL}/tickets/{ticket_id}", headers=headers)
    ...
    update_response = requests.patch(
        f"{BASE_URL}/tickets/{ticket_id}", json=update_data, headers=headers
    )
...
def test_code_generation_flow(token: str, ticket_id: str):
    ...
    generate_response = requests.post(
        f"{BASE_URL}/code/generate", json=generation_data, headers=headers
    )
    assert generate_response.status_code == 200
    code_id = generate_response.json()["id"]

    # Get generated code
    get_response = requests.get(f"{BASE_URL}/code/{code_id}", headers=headers)
    assert get_response.status_code == 200
...
def test_test_generation_flow(token: str, code_id: str):
    ...
    # Generate tests
    test_data = {"code_id": code_id, "test_type": "unit"}
    generate_response = requests.post(
        f"{BASE_URL}/tests/generate", json=test_data, headers=headers
    )
    assert generate_response.status_code == 200
    test_id = generate_response.json()["id"]

    # Run tests
    run_response = requests.post(f"{BASE_URL}/tests/{test_id}/run", headers=headers)
...
def test_version_control_flow(token: str):
    ...
    # Create branch
    branch_data = {"name": "test-branch", "base": "main"}
    branch_response = requests.post(
        f"{BASE_URL}/git/branches", json=branch_data, headers=headers
    )
    ...
    commit_response = requests.post(
        f"{BASE_URL}/git/commit", json=commit_data, headers=headers
    )
...
def test_cicd_flow(token: str, branch_name: str):
    ...
    pipeline_response = requests.post(
        f"{BASE_URL}/cicd/pipelines", json=pipeline_data, headers=headers
    )
    ...
    run_response = requests.post(
        f"{BASE_URL}/cicd/pipelines/{pipeline_id}/run", headers=headers
    )
    assert run_response.status_code == 200

    # Get pipeline status
    status_response = requests.get(
        f"{BASE_URL}/cicd/pipelines/{pipeline_id}", headers=headers
    )
