...
MOCK_PIPELINE = {
    ...
    "branch": "feature/auth",
    "stages": [
        {"name": "test", "command": "pytest tests/", "timeout": 300},
...
async def auth_token(client):
    ...
    response = await client.post(
        "/token",
        data={"username": MOCK_USER["email"], "password": MOCK_USER["password"]},
    )
    assert response.status_code == 200
...
async def test_full_workflow(client, auth_token):
    ...
    response = await client.post(
        "/code/", json={**MOCK_CODE_GENERATION, "ticket_id": ticket_id}, headers=headers
    )
    assert response.status_code == 200
    ...
    response = await client.post(
        "/tests/", json={**MOCK_TEST_GENERATION, "code_id": code_id}, headers=headers
    )
    assert response.status_code == 200
    ...
    response = await client.post(
        "/version-control/commit/", json=MOCK_COMMIT, headers=headers
    )
    assert response.status_code == 200
...
async def test_error_handling(client, auth_token):
    ...
    response = await client.post(
        "/version-control/commit/", json=invalid_commit, headers=headers
    )
    assert response.status_code == 422
    ...
    response = await client.post(
        "/cicd/pipeline/", json=invalid_pipeline, headers=headers
    )
    assert response.status_code == 422
