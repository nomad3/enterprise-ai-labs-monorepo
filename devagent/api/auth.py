...
async def get_current_active_superuser(current_user: User = Depends(get_current_user_dependency)):
...
async def register_user(user_in: UserCreate, db: AsyncSession = Depends(get_session)) -> RegisterResponse:
    """
...
async def login_json_for_ui(
    ...
    return AuthSuccessResponse(access_token=access_token, user=UserResponse.from_orm(user))
...
async def login_form_for_oauth_token(
    ...
    if not user.is_active:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Inactive user (form data)")
...
async def read_users_me(current_user: User = Depends(get_current_user_dependency)) -> UserResponse:
    """
    Get current user.
    """
...
@router.post("/login/email", response_model=Token) # Keep this for UI compatibility for now
