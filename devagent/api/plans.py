...
async def create_solution_plan(
    ...
    requirements = await db.query(Requirement).filter(Requirement.ticket_id == ticket_id).all()
    if not requirements:
        raise HTTPException(status_code=400, detail=f"No requirements found for ticket {ticket_id}")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error creating solution plan: {str(e)}")
...
async def get_ticket_plan(
    ...
    """
    # Query plan
    plan = await db.query(SolutionPlan).filter(SolutionPlan.ticket_id == ticket_id).first()
    if not plan:
        raise HTTPException(status_code=404, detail=f"No plan found for ticket {ticket_id}")
