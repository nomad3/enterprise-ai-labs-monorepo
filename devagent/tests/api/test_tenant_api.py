...
DATABASE_URL_FOR_TESTS = os.getenv("DATABASE_URL", "sqlite+aiosqlite:///./test_api.db") # Match docker-compose env var

engine_test = create_async_engine(DATABASE_URL_FOR_TESTS, echo=False) # echo=False for less verbose test output
...
@pytest_asyncio.fixture(scope="function")
async def create_test_tenant(db_session: AsyncSession, tenant_data: Dict[str, Any]) -> TenantRead:
...
async def test_list_tenants_with_data(client: AsyncClient, db_session: AsyncSession):
    ...
    data = response.json()
    assert len(data) == 2
    assert data[0]["name"] == default_tenant_data_1["name"] or data[1]["name"] == default_tenant_data_1["name"]
    assert data[0]["name"] == default_tenant_data_2["name"] or data[1]["name"] == default_tenant_data_2["name"]

@pytest.mark.asyncio
async def test_list_tenants_pagination(client: AsyncClient, db_session: AsyncSession):
    # Create 3 tenants
    db_session.add_all([Tenant(**default_tenant_data_1), Tenant(**default_tenant_data_2), Tenant(**default_tenant_data_3)])
    await db_session.commit()
...
@pytest.mark.asyncio
async def test_list_tenants_filter_by_name(client: AsyncClient, db_session: AsyncSession):
    db_session.add_all([Tenant(**default_tenant_data_1), Tenant(**default_tenant_data_2), Tenant(**default_tenant_data_3)])
    await db_session.commit()
...
@pytest.mark.asyncio
async def test_list_tenants_filter_by_is_active(client: AsyncClient, db_session: AsyncSession):
    db_session.add_all([Tenant(**default_tenant_data_1), Tenant(**default_tenant_data_2), Tenant(**default_tenant_data_3)])
    await db_session.commit()
...
@pytest.mark.asyncio
async def test_list_tenants_filter_by_subscription_tier(client: AsyncClient, db_session: AsyncSession):
    db_session.add_all([Tenant(**default_tenant_data_1), Tenant(**default_tenant_data_2), Tenant(**default_tenant_data_3)])
    await db_session.commit()
...
async def test_update_tenant_not_found(client: AsyncClient):
    ...
    assert response.status_code != 200 # General check, specific code depends on error handling
